...
void StatementGenerator::emitCase(PascalParser::CaseStatementContext *ctx)
{
	...
	for (PascalParser::CaseBranchContext *Branch : ctx->caseBranchList()->caseBranch())
	{
		if(Branch->children.size() > 0)
		{
			...
			for(PascalParser::CaseConstantContext *constant: Branch->caseConstantList()->caseConstant())
			{
				...
				string Constant = type->getForm() == Form::ENUMERATION ? to_string(constant->value) : constant->getText();
	...
	for(pair<pair<string,Typespec*>, int> Constants : ConstantList)
	{
		...
		else if(Constants.first.second == Predefined::integerType || Constants.first.second->getForm() == Form::ENUMERATION)
